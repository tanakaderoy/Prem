...
class MatchNetworkAdapter: NSObject, URLSessionDelegate {
    ...
    private let endpoint =  URL(string: "https://api.football-data.org/v2/competitions/2021/matches?matchday=31")!
        ...
        let task = session.dataTask(with: request){ (data, response, error)  in
            ...
            if let data = data {
                ...
                do{
                    ...
                    print("error with JSON decoding: \(error.localizedDescription)")
                }
